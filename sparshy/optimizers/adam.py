...
class Adam:
    ...
    def add_params(self, w_shape: Tuple[int, int], b_shape: Tuple[int, int]):
        """
        Initialize first and second moment estimates with zeros for a new layer.
    ...
    def backward(
        ...
        """
        Perform backward propagation using Adam optimizer.
    ...
    def __get_param_updates(
        ...
        """
       Compute weight and bias updates using Adam optimization.
       ...
       """
        ...
        w_update = lr * mw_hat / (np.sqrt(vw_hat) + self.epsilon)
        b_update = lr * mb_hat / (np.sqrt(vb_hat) + self.epsilon)
